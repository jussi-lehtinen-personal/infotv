...
const SCORING = {
  ...
  // DEFENDER goal/assist multiplier — REMOVED 2026-08-31 (`defenderMult: 1` = no boost).
  // The ×1.5 was ONE lever to make defenders worth picking; it was replaced by the "RX"
  // PRICING model (bandPricesFrom in ahmaliiga.js: defenders priced vs the best defender→60,
  // but absolute merit can still reach 75). Pricing is fairer under imperfect position tags:
  // a scoring boost gave a TAGGED defender +48% points while an identical UNTAGGED (KP) one
  // got none → distorted standings; pricing only shifts VALUE, and points stay honest for
  // everyone (goal=3 regardless of tag). Kept as a constant (not deleted) so it's trivially
  // re-tunable; `1` makes the multiply a no-op. See project_ahmaliiga_balance (R60/RX).
  player: { goal: 3, assist: 2, defenderMult: 1 },
...
function goaliePoints(report, ctx) {
  ...
  // Kept in step with the same walk in src/pages/game.js (goalieGA).
  const whoAt = (tt) => {
    let w = tl[0].who;
