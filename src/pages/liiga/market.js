...
export default function LiigaMarket() {
  ...
  const [roundLive, setRoundLive] = useState(false); // a round is in progress → "Jakso" shows live points
  const [ownedIds, setOwnedIds] = useState(null); // ids of cards in my squad → highlighted

  useEffect(() => {
    let cancelled = false;
    getAhmaliigaCards()
      .then((d) => { if (!cancelled) { setCards(d.cards || []); setSettled(!!d.settled); setRoundLive(!!d.roundLive); } })
  ...
  return (
    <Screen>
      ...
      {cards == null ? (
        ...
        <CardList cards={cards} settled={settled} roundLive={roundLive} ownedIds={ownedIds}
