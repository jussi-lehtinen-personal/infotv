...
// A new SW WAITS after install — it does NOT auto-skip-waiting. Activation is
// driven by an explicit SKIP_WAITING message (the "Päivitä" bar taps it, and the
// registration also taps it on resume). This is deliberate: an unconditional
// top-level self.skipWaiting() here fought with the update-prompt flow — the new
// worker auto-activated + reloaded while the bar was being shown, so "Päivitä"
// reappeared endlessly and reg.waiting was already gone when tapped. Blank-shell
// self-heal is handled by the NETWORK-FIRST navigation route below, not by
// skip-waiting. clientsClaim() still lets the worker control the page the moment
// it does activate.
clientsClaim();
