...
- **U2** (F3.1) **Peliryhmä (Musta/Oranssi) ikäryhmän lisäksi ottelunäkymiin.** Ahman peliryhmä pudotetaan usealla pinnalla. ⚠️ **Data on jo saatavilla** — lähde on **tulospalvelun pelidata** (ei Jopox): väri on `level`issä ("U13 Oranssi") tai joukkueen nimessä, ja **`gameTeamKey(g)`** (`events.js`, peilaa backendin **`teamKey`**iä — se jaettu ikäluokka+väri-funktio) laskee jo esim. "U15 Musta". Fix = **pelkkä näyttö**, kaksi korjauskohdetta:
