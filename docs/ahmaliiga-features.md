...
- **U1** (F7.1) **Nykyinen (live) jakso ranking-sivulle.** Nyt `ranking.js` "Viime jakso" = viimeisin **RATKAISTU** jakso (koodikommentti: *"the in-progress round has no standings yet"*, `settledNo = curNo − 1`); käynnissä oleva jakso puuttuu kokonaan. Lisää **"Nyt"/"Live"-scope** joka näyttää kuluvan jakson **alustavan** järjestyksen. **Data on jo olemassa:** `round.js` laskee oman `progress.livePoints`in per manageri (`getAhmaliigaRoundProgress` → per-peli-pisteet box scoreista, sama `computeRoundResults`-logiikka kuin settlementissä). Tarvitaan: **live-leaderboard-aggregaatti** (per-peli-pisteet KAIKILLE managereille kuluvalta jaksolta) + **cache** (⚠️ [[T2]]: kaikkien managerien live-laskenta on raskas → cachetettava esim. 30–60 s, tai lasketaan settlement-pollerin sivutuotteena). Merkitse selvästi "alustava · päivittyy".

- **U2** (F3.1) **Timelineen ryhmätunnus (Musta/Oranssi) ikäryhmän lisäksi.** `EventRow` näyttää ikäryhmän muttei **peliryhmää** (Musta/Oranssi). ⚠️ **Data on jo timelinessä** — lähde on **tulospalvelun pelidata**, ei Jopox: **`gameTeamKey(g)`** (`events.js`, peilaa backendin **`teamKey`**iä — se jaettu funktio joka jakaa Ahma-joukkueen ikäluokkaan + väriin, käytössä monessa paikassa) laskee jo esim. "U15 Musta" pelistä (kortti↔peli-mätsäystä varten). Ainoa puute: **`gameTitle` pudottaa värin** (käyttää `AHMA_AGE`:a → "U15 vs Pelicans"). Fix = **pelkkä näyttö**: näytä `gameTeamKey`n väri `EventRow`ssa (esim. "U15 Musta vs Pelicans" tai erillinen ryhmä-chip). Ei uutta datalähdettä, ei privacy-kysymystä.

- **U3** (bugi · HARD RULE) **Timelinen oranssit pallot eivät keskity korttiin.** `round.js` `TimelineTab`: pallon pystysijainti on kova `mt: "16px"` (rivi ~158) joka ei osu `EventRow`n optiseen keskilinjaan. Korjaa niin että pallo + rivin sisältö jakavat keskilinjan ([[feedback_icon_text_centered]] — toistuvin palaute). CSS-only.

- **U4** (F4 · F7.3) **Jakson pelit + live-pisteet näkyviin kortteja vaihtaessa — ja sivujen yhdistäminen?** Kolmiosainen:
  1. *Näytä jakson pelit squad-flowssa* — jotta kortteja voi vaihtaa viisaammin, näe mitä pelejä jaksolla ylipäänsä on. Info on jo `predict.js`n peli-dropdownissa JA `round.js` Aikajanassa → tuo se näkyvästi **Oma joukkue** -sivulle (`edit.js`) tai linkitä sinne.
  2. *Live-pisteet pelien ratketessa* — **OSIN JO TEHTY:** `round.js` Aikajana näyttää `progress.livePoints` + `perGame` per peli LIVENÄ. Käyttäjä ei löytänyt sitä ("näkee vasta kun jakso ratkeaa") → **löydettävyys-/sijaintiongelma, ei puuttuva feature.** Nosta squad-sivulle / dashboardille.
  3. *Yhdistä kaksi sivua?* — AVOIN DESIGN-kysymys. Todennäk. tarkoitus **Oma joukkue (squad/`edit.js`) ↔ Jakso (`round.js`: Aikajana/Tulokset)**: kortteja vaihtaessa haluat nähdä jakson pelit + live-pisteet samassa näkymässä. (`round.js` yhdisti jo Tulokset+Aikajana keskenään.) Vaihtoehto: Jakso ↔ Ranking. Suositus: **squad + round yhteen** = rakennus + eteneminen on yksi luuppi. Päätä design-vaiheessa.
