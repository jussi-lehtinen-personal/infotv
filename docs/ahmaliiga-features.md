...
- **U5** (F3.4 · F3.7 · B6) **Kortin hinta päivittyy heti pelatun pelin jälkeen** (idea 2026-07-25). Nyt uudelleenbändäys (reband) tapahtuu vain **jakson vaihteessa** → kortin (pelaajan/joukkueen) hinta liikkuu vasta jakson lopussa. Toive: hinta reagoi **heti** kun kortin peli on pelattu + pisteytetty → "pörssimeta" ([[B6]]) tuntuu elävältä kesken jaksonkin (osta halvalla ennen kuin muut huomaavat). Vaatii: per-peli- (tai lyhyt-sykli-) hinnanpäivitys settlement-pollerissa / live-tikissä + **`CardHistory`-snapshot** tiheämmin (F3.7). ⚠️ Varmista: **lock-in säilyy** (ostohinta lukittu, F4.4 — pidetyn kortin arvonnousu ei riko squadia) + hinta ei heilu liikaa yhdestä pelistä (vaimennus/cap). Suunnittele ennen implementaatiota.
...
- **U7** (T2) **Dashboard (`home.js`) JA Oma joukkue (`edit.js`) latautuvat välillä hitaasti — skeleton/progress.** (Palaute 2026-07-25.) Oireet: (1) **Dashboard** — joskus näkyy vain staattinen osa ("Kokoa kortisto ja nouse mestariksi" + divider + WhatsApp-nappi); Ahmaliiga-logo + jaksokortit puuttuvat kunnes data latautuu. (2) **Oma joukkue** — lataus kestää usein monta kertaa; kortit voisi näyttää **heti** ja niiden sisään pelkkä progress-spinneri. Fix (molemmat): **skeleton-/placeholder-runko renderöityy HETI** (kehykset + spinneri/progress sisällä), täyttyy datan tullessa → ei tyhjää ruutua. Liittyy [[T2]] (edit.js sai 1. erän: roundProgress irti renderöinnistä + client-cache — ilmeisesti ei riitä, kortit itse odottavat vielä). Profiloi pullonkaulat (`getAhmaliigaState`/`getMySquad`/`getCards`/roundProgress) → piirrä runko heti (cachesta tai tyhjänä), lataa loput taustalla.

- **U8** (F7.3) **Per-kortti-rivin laajennus (expand) → mistä pisteet tulivat.** (Palaute 2026-07-25.) `round.js` Tulokset-välilehden per-kortti-rivi voisi **avautua** näyttämään mitä kortti (pelaaja/joukkue) teki **kussakin ottelussa** ja mistä pisteet kertyivät (maali/syöttö/voitto/nollapeli/…). Data: settlementissä on `reasons` per kortti; live-puolella `perGame` per peli. Tarvitaan **per-kortti-per-peli-erittely** (kortti × ottelu → pisteet + syy) — osin olemassa (`computeRoundPoints` reasons), mutta ei per-peli-per-kortti-muodossa clientille. Suunnittele: laajenna `roundProgress`/summary palauttamaan per-kortti pelilista (peli, pisteet, syy), ja tee Tulokset-rivistä accordion.
