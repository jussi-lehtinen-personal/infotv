...
## Balanssi v2 (SOVITTU 2026-07-19, EI RAKENNETTU) — seuraavaan testipeliin

Johdettu **ensimmäisen kokonaisen testikauden analyysistä** (palautettu backup → lokaali Azurite; `tools/analyze-*.js` + `simulate-*.js`). Täysi "miksi" + evidenssi: **`project_ahmaliiga_balance`** (muisti). Nämä **päivittävät** yllä olevia F-lukemia — toteutetaan + **backtestataan luvut** ennen seuraavaa (live-synkattua, `roundConfig`) testipeliä. Havaittu ydin: joukkueet olivat liian halpoja + tasahintaisia (arvo tuli HINNASTA, ei volyymista — `analyze-teamgames.js` kumosi volyymiteorian: Naiset pelasi VÄHITEN 22 peliä mutta korkein 3.9 p/peli); voittaja voitti seura-tietämyksellä (draft-edge), ei exploitilla (kapteeni ×1.5 eikä Olander@70 muuta lopputulosta).

- **B1** (F2.2/F2.5) **Jaksot = ma–su kalenteriviikot** (oli pe-loppu/la-alku). Lukitus = oman squadin **ensimmäinen peli** → "päivitä ennen omaa peliäsi" = luonteva viikkoluuppi (julkiset kokoonpanot tulevat ennen pelejä).
...
- **B10** (F3.2) **Pelaajapooliin viime kauden U15 (2010-syntyneet).** `PLAYER_AGES` nyt `[Edustus, Naiset, U20, U18]` (BOTH `api/src/lib/roundResults.js` JA `tools/lib/model.js` — pidä synkassa). 2010-syntyneet (viime kauden U15, enimmäkseen tulevan kauden U18) mukaan seuraavaan replayhin. **Ei syntymävuosikenttää Jopoxissa EIKÄ tulospalvelussa** → **nimimätsäys**: tulevan kauden U18 Jopox-rosteri ↔ viime kauden U15 box-score-pelaajat → vain osumat mukaan (2011-syntyneet jäävät U18-rosterin ulkopuolelle → karsiutuvat luonnostaan). Pisteytä viime kauden U15-peleistä.
