...
- **T2** (ROADMAP 2026-07-18) **Sivujen / latauksen optimointi.** Oire: **"Oma joukkue"** (`/ahmaliiga/squad`) latautuu monta kertaa ja kestää. Todennäköiset syyt: (a) jokainen mount hakee raskaita endpointteja uudelleen **ilman client-cachea** — `edit.js` tekee `Promise.all` 4 kutsua joista **`roundProgress` hakee box scoret** (hidas); (b) ei jaettua Ahmaliiga-client-cachea (toisin kuin `seasonGamesCache`); (c) dashboard hakee nyt myös voucherit + progressin. TODO: **profiloi ensin** mihin aika menee (todennäk. roundProgress-box-scoret); kevyt **module-scope-cache + TTL** ahmaliiga-endpointeille (state/cards/squad/roundProgress), squad-sivulle **kevyempi per-kortti-pistelähde** (ei koko box-score-roundProgressia jos vain `perCard` tarvitaan), skeleton/optimistinen render, rinnakkaisten kutsujen dedupe. Koskee erityisesti squad + dashboard.
