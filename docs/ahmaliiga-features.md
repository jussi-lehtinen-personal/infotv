...
- **U12** (F6.2 · B-balanssi) **Hyökkääjä-vinouma — puolustuksen/kollektiivin palkitseminen (positiodata SITTENKIN saatavilla Jopoxista).** (Palaute 2026-07-31, Jussi + Jani.) Havainto: **yksilöpoiminta = avain voittoon**, hyökkääjät **OP** ja puolustajien poiminta harvinaista, koska pisteytys palkitsee vain maali 3 / syöttö 2 (ei +/- eikä kentän puolustuksen krediittiä). Janin idea: **puolustaja saisi pisteitä joukkueen vähäisistä päästetyistä maaleista** (0–2 / nolla).
  - **💡 Positiodata ON saatavilla Jopoxista (korjaus 2026-07-31 probe):** Jopox-rosterin `players`-**ryhmän nimi** (`group.name`) voi olla pelipaikka. **U18 (9949) käyttää tätä jo:** `"Maalivahti"(2) · "Puolustaja"(3) · "Hyökkääjä"(7)` (+ `null`(12) ryhmittelemättä). Muut ikäluokat merkkaavat nyt vain "Maalivahti", loput `null`. → **puolustaja-tunnistus on mahdollinen nimimätsäyksellä** (sama kuvio kuin U15-callupit: `roster.js fetchJopoxRosterNames` + `model.js normName`), kun ryhmän nimi tuodaan mukaan. **Pelaajakortit ovat vain aikuisikäluokille** (Edustus/Naiset/U20/U18 = `PLAYER_AGES`) → **vain ~4 joukkuetta pitää ylläpitää P/H-ryhmiä**, ja U18 tekee sen jo. Erittäin hoidettavissa.
  - **Riippuvuus = seuran ylläpito:** vaatii että ne 4 joukkuetta pitävät Puolustaja/Hyökkääjä-ryhmät ajan tasalla Jopoxissa (nyt vain U18 osittain; 12/24 vielä `null`). Ryhmittelemättömille → **fallback** (alla).
  - **Toteutusvaihtoehdot:** **(1) positiopohjainen** (kun Jopox-ryhmä tiedossa): puolustaja saa nollapeli/vähämaali-bonuksen (esim. nolla +3, ≤2 päästettyä +1). **(2) fallback ilman positiota:** esiintymis-+tulosbonus — kuka tahansa pukeutunut pelaajakortti joukkueessa joka voitti/piti nollan → pieni +1 (palkitsee puolustajat & nelosketjun joukkuemenestyksestä). Yhdistelmä: käytä positiota jos on, muuten fallback.
  - **Sim:** kvantifioi hyökkääjä-vinouma (% pisteistä maali/syöttö vs. joukkue/puolustus), testaa positio- ja fallback-bonuksen vaikutus. **Jussi ajaa simin** — **yhdistä U9 + U11a + U12 = YKSI balanssi-sim** (sama työkalupohja: restore-backup + `analyze-teamgames`/`backtest-v2`, CardHistory + box-score-rosterit + Jopox-rosterit).

- **U13** (laatu · onboarding) **Codex/Playwright-testiraportin löydökset (Rami Motturi 2026-07-31).** Automaatiotesti kävi läpi koko ydinpolun (passkey-tili → kortisto → kapteeni → veikkaus) **onnistuneesti**; API:t OK. Löydökset:
  - **#1 Google-kirjautuminen ORIGIN-virhe** (⚠️ oikea, ei koodi): `[GSI_LOGGER]: The given origin is not allowed for the given client ID` + GSI 400. Google OAuth -clientin **sallittuihin JavaScript-origineihin puuttuu `https://gamezone.kiekko-ahma.fi`** (client `GOOGLE_CLIENT_ID`, tarjoillaan `authConfig`ista). **Korjaus: Google Cloud Console → APIs & Services → Credentials → OAuth client → Authorized JavaScript origins → lisää prod-osoite** (+ tarvittaessa custom-domainit info./gamezone.). Vaikutus: Google-login + linkitys epäonnistuu oikeilla käyttäjillä (passkey toimii). **Seuraus #4:** ilman Googlea monilaite­palautus vaikeaa → korjaantuu tällä.
  - **#2a "Beta alkaa ensi viikonloppuna" vanhentunut** — ✅ **KORJATTU 2026-07-31**: `promo.js` (`/ahmaliiga/info`) → "Beta on käynnissä" + "Tee käyttäjä ja pääset heti mukaan pelaamaan".
  - **#2b Simuloitu kausi hämää** (UX): sisäänkirjautuneena `simDate` on helmikuu 2026 (replay 2025–26 kaudesta) → merkitse selvästi **"simuloitu testikausi"** jottei päivämäärä hämää. Pieni selite dashboardille / tervetulosivulle.
  - **#3 First-load service worker -reload söi ensimmäisen klikkauksen** (🟡 varmista): ensimmäisellä käynnillä SW asentui + sivu latautui uudelleen kesken "Luo uusi tili" -klikin → modaali ei auennut. Toistui vain SW:n asennuksen aikana (todennäk. automaatioajoitus, mutta **testaa aito ensikäynti** selaimessa jossa SW asentuu 1. kertaa; `skipWaiting`/`clients.claim` voi aiheuttaa controllerchange→reload). Jos oikea → viivytä SW:n `clients.claim` / älä pakota reloadia ensi-interaktion aikana.

- **U14** (turvallisuus · abuse) **Kiusanteko- ja hyökkäysriskien kovennus (Rami Motturi -raportti 2026-07-31).** Perusauth + admin-roolitus **OK matalan riskin tarkistuksissa** (tavallinen käyttäjä → 403 admin-poluille; IDOR:ia ei osoitettu peruspyynnöillä). Väitteet **verifioitu koodista 2026-07-31.**
  - ✅ **KORJATTU HETI:** (a) **julkiset production source mapit** — vahvistettu (prod `main.*.js.map` HTTP 200); `build`-skripti oli pelkkä `react-scripts build` → lisätty `.env` `GENERATE_SOURCEMAP=false` (koskee SWA-deployta). (b) **turvaotsikot** `staticwebapp.config.json`iin: `X-Content-Type-Options: nosniff` + `Referrer-Policy` (turvalliset, ei riko mitään).
  - **P1 — Rate limitit (VAHVISTETTU puuttuvan, `api/src`:ssä ei mitään):** rekisteröinti + kirjautuminen + KAIKKI mutatoivat PUT/POST. Estää massatilit + julkisten endpointtien kuormituksen. Toteutus: per-IP-laskuri (Table Storage) Functionsissa TAI Cloudflare/Azure Front Door -kerros edessä. **+ beta-portti:** kutsukoodi tai progressiivinen CAPTCHA rekisteröintiin.
  - **P2 — CSP + loput turvaotsikot:** ei CSP:tä nyt. Vaatii **huolellisen laadinnan + testauksen** (MUI inline-tyylit, Google GSI, CF-worker, kuva­proxy, kioskin kamera `kiosk.js`) → ei sokkona (voi rikkoa sovelluksen). Lisää samalla `X-Frame-Options` (varmista ensin ettei InfoTV-näyttöjä upoteta iframeen) + `Permissions-Policy` (salli `camera=(self)` kioskille).
  - **P2 — Moderointi/ban-työkalut:** loukkaavat nimimerkit/avatarit → admin voi nimetä uudelleen / piilottaa / bannata. Ei nyt mitään.
  - **P2 — Omistajuus-/roolitarkistusten audit:** käy läpi JOKAINEN mutatoiva endpoint (squad, prediction, reservations, vouchers/redeem, manage*) — `requireAuth`→userId-scope + rooli. Perus kunnossa, mutta täysi IDOR-audit tekemättä (erillinen luvitettu pentest suositeltava).
  - **P2 — Audit-loki + hälytykset** kriittisille: massarekisteröinnit, roolimuutokset, backup/restore, Ahmaliiga reset/settle, voucher-lunastus. (Osa on: `AhmaliigaSquadLog`.)
  - **P3 — `localStorage`-token (XSS):** vahvistettu (`authClient` localStorage). Ensisijainen mitigointi = CSP (P2). Dokumentoi/hyväksy tietoinen valinta.
  - **P3 — Julkiset data-endpointit cache/limit** (etenkin 3. osapuolen data): tulospalvelu jo worker-cachattu; varmista omat julkiset GETit.
  - **Ristiin U13 #1:** Google-origin-korjaus + **tilinpalautuspolku dokumentoitava** (mitä jos passkey katoaa eikä Googlea linkitetty).
