...
- **U2** (F3.1 · feed-subgroups) **Timelineen ryhmätunnus (Musta/Oranssi) ikäryhmän lisäksi.** `round.js`/`events.js` `EventRow` näyttää ottelun ikäryhmän muttei **peliryhmää** (Musta/Oranssi) — puuttuu. Lähde on jo ratkaistu: **autentikoitu Jopox members-API** (`api/src/lib/jopox.js` → `fetchMemberSubGroups`; feed käyttää jo `ev.subGroups`, ks. `project_feed_subgroups`). Kytke peliryhmä Ahmaliigan jakson peleihin → näytä `EventRow`ssa yhtenäisesti (esim. "U15 · Musta"). Privacy: vain ryhmälabel, ei henkilödataa.
